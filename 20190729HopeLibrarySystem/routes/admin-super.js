...
router.route('/admin-user/search').get(function(req,res){
    if(!req.session.adminID || !req.session.adminSign){
        res.redirect("/hopelibrary/admin/login");
        return;
    }
    adminDB.selectMessage(req.session.adminID, (rows) => {
        ...
        const [userName, userImg, userPermission] = [admin.adminName, admin.adminImgSrc, admin.adminPermissions];
        const query1 = 'SELECT adminName,adminPermissions,adminID FROM hopeadmin WHERE adminName LIKE "%'+ word+'%"'
        userDB.query(query1,(rows)=>{
            let admins = rows
            const query2 = 'SELECT readerName,readerGroup,readerID'
                      + ' FROM hopereader'
                      + ' WHERE (readerName LIKE "%'
                      + word
                      + '%" OR readerGroup LIKE "%'
                      + word
                      + '%")';
            userDB.query(query2, (rows) => {
                let reader = rows;
                let user = admins;
                user=user.concat(reader)
                setSession(req,{adminID:admin.adminID,adminSign: true}); 
                res.render("admin-super/admin-super-search",{userName,userImg,userPermission,firstPath:'user',secondPath:'search',user});
            });
        })
    });
})
