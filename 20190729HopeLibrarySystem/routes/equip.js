...
router.route("/").get(function(req,res){
    ...
    userDB.selectMessage(userID, (rows) => {
        ...
        equipDB.orderItems('equipLeft DESC,equipHopeID ASC',pageStart, pageEnd,  (rows) => {
            const equip = rows;
            equipDB.countItems('equipNum', (rows) => {
                let equipNum = Math.ceil(rows[0].equipNum/12);
                setSession(req, {userID: user.readerID, userSign: true});
                res.render("user/user-equip",{userImg:userImg,userName:userName,userPermission:userPermission,firstPath:'reservation',secondPath:'',equip,equipNum,equipPage:pageNum});
            })
        });
    });
});
router.route('/search').get(function(req,res){
    ...
    userDB.selectMessage(userID, (rows) => {
        ...
        equipDB.search(word,(rows)=>{
            const equip = rows;
     
            setSession(req,{userSign:true,userID: req.session.userID});
            res.render("user/user-search-equip",{userName,userImg,userPermission,firstPath:'reservation',secondPath:'search',equip})
        })
    });
