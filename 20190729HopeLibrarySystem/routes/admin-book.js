...
router.route("/book-modify/:bookID").get(function(req,res){
    ...
    let setDataJson;
    const tempImgSrcs = req.body.bookImgSrc.toString();
...
router.route('/admin-book/search').get(function(req,res){
    if(!req.session.adminID || !req.session.adminSign){
        res.redirect("/hopelibrary/admin/login");
        return;
    }
    adminDB.selectMessage(req.session.adminID, (rows) => {
        ...
        const [userName, userImg, userPermission] = [admin.adminName, admin.adminImgSrc, admin.adminPermissions];
        const query = 'SELECT bookName,bookID,bookHopeID,bookLeft'
                      + ' FROM hopebook'
                      + ' WHERE  (bookName LIKE "%'
                      + word
                      + '%" OR bookPress LIKE "%'
        ...
        bookDB.query(query,(rows)=>{
                ...
                bookDB.query(query, (rows) => {
                    ...
                    setSession(req,{adminID:admin.adminID,adminSign: true});
                    res.render("admin-book/admin-book-search",{userName,userImg,userPermission,firstPath:'book',secondPath:'search',book,borrower});
                });
        })        
    })
})
