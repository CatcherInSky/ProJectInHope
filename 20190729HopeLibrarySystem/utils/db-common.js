...
operate.prototype.search = function(database,column,word,callback){

		let action = 'SELECT * FROM '+ database +' WHERE ';
		for(let i = 0;i<column.length;++i){
			if(i==0){
				action += column[i] + ' LIKE "%' + word + '%"'
			}else{
				action += ' OR '+ column[i] + ' LIKE "%' + word + '%"'
			}
